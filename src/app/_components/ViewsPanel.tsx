...
type ViewsPanelProps = {
  ...
  onCreate: () => void;
  onClose: () => void; // still passed, but only used by the parent toggle
...
export default function ViewsPanel({
  ...
  onSelect,
  onCreate,
  onClose, // kept for API symmetry; not used internally anymore
  leftOffset = 0,
  bottomOffset = 0,
  onHoverIn,
  onHoverOut,
  ...
  return (
    <>
      {/* Mobile backdrop is now visual only; it no longer closes the panel on click */}
      <div
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="px-2">
            <button
              type="button"
              onClick={onCreate}
              className="mb-2 flex w-full cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left text-[13px] text-neutral-700 hover:bg-neutral-50"
            >
            ...
            {filtered.map((v) => {
              ...
              return (
                <button
                  ...
                  className={
                    "flex w-full cursor-pointer items-center gap-2 rounded px-3 py-2 text-left text-[13px] " +
          ...
          <div className="pointer-events-none relative">
            <button
              type="button"
              onClick={onCreate}
              className="pointer-events-auto absolute bottom-4 left-4 inline-flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-neutral-100 text-neutral-700 ring-1 ring-neutral-300 hover:bg-neutral-200"
              title="New view"
            >
              +
            </button>
