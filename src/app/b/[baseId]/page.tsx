...
export default function BasePage() {
  ...
  const totalRecords = useMemo(() => {
    // grab `total` from the very first page if it's there
    const firstPage = rowsQ.data?.pages?.[0];
    if (!firstPage) return 0;
    // fall back to current loaded length if total is missing for some reason
  ...
  const handleAddNumberColumn = () => {
    ...
    createColumn.mutate({
      tableId,
      name: "Number", // always literally "Number"
      type: "NUMBER",
    });
  };
